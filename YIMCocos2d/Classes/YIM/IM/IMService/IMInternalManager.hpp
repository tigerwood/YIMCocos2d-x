...
public:
    ...
    // 获取自己位置回调
    virtual void OnUpdateLocation(YIMErrorcode errorcode, GeographyLocation* location) override ;
    // 获取附近目标回调
    virtual void OnGetNearbyObjects(YIMErrorcode errorcode, std::list<RelativeLocation*> neighbourList, unsigned int startDistance, unsigned int endDistance) override;
    ...
    //发送语音消息回调
    virtual void OnSendAudioMessageStatus(XUINT64 requestID, YIMErrorcode errorcode, const XCHAR* text, const XCHAR* audioPath, unsigned int audioTime)override;
    
    void OnSendAudioMessageStatusChange(XUINT64 requestID, YIMErrorcode errorcode, const XCHAR* text, const XCHAR* audioPath, unsigned int audioTime, bool isFinish);
    
    
    //收到消息
    virtual void OnRecvMessage(IYIMMessage* message) override;
    //获取消息历史纪录回调
    virtual void OnQueryHistoryMessage(YIMErrorcode errorcode, const XCHAR* targetID, int remain, std::list<IYIMMessage*> messageList)override;
    //获取房间历史纪录回调
    virtual void OnQueryRoomHistoryMessage(YIMErrorcode errorcode, std::list<IYIMMessage*> messageList)override;
    //语音上传后回调
    virtual void OnStopAudioSpeechStatus(YIMErrorcode errorcode, IAudioSpeechInfo* audioSpeechInfo)override;
    ...
    virtual void OnDownload(XUINT64 messageID, YIMErrorcode errorcode, const XCHAR* savePath)override;
    
    ///IYIMContactCallback
    //获取最近联系人回调
    virtual void OnGetRecentContacts(YIMErrorcode errorcode, std::list<const XCHAR*>& contactList) override;
    //获取用户信息回调(用户信息为JSON格式)
    virtual void OnGetUserInfo(YIMErrorcode errorcode, const XCHAR* userInfo)override;
    //查询用户状态回调
